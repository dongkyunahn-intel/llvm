...
template<typename T, unsigned N>
uint64_t inline getLaneAddr(__SEIEED::vector_type_t<T, N> addrMsg, unsigned lane_id)
{
  // (matrix_ref<T, R, C> addrMsg)
  // vector_ref<uint64_t, 1> addr_ref = addrMsg.template select<1, 1, 2, 1>(0, 2 * lane_id).template format<uint64_t, 1, 1>();
  // return addr_ref(0);
  throw cl::sycl::feature_not_supported();
}
...
__esimd_raw_send_load(uint8_t modifier, uint8_t execSize,
  ...
  bool Transposed = raw_send::getMsgField(msgDesc, raw_send::msgField::TRANSPOSE);
       ...
       xBase += sizeofT * Width)
  {
    if (Transformed == true)
    {
      ...
      { /// Transform only load
        ...
        for (int u = 0; u < Height; u += elems_per_DW, yRead += SurfacePitch * elems_per_DW)
        {
      ...
      { /// Transform & Transpose load
        int xRead = xBase;
        for (int v = 0; v < Width; v += elems_per_DW, xRead += sizeofT * elems_per_DW)
        {
          if ( (xRead < 0) || (xRead >= SurfaceWidth) )
          {
            // Horizontally out-of-bound
    ...
    {
      if (Transposed == false)
      { /// Linear load - no transform, no transpose
        int yRead = Y * SurfacePitch;
        for (int u = 0; u < Height; u += 1, yRead += SurfacePitch)
        {
          if ( (yRead < 0)
               || (yRead >= SurfacePitch * SurfaceHeight))
          {
            // Vertically Out-of-bound
